...
function AnswerVoteControls({
  ...
  return (
    <div className="flex items-center gap-1 rounded-2xl border border-[#e1e9e6] bg-[#f7f9f8] p-1 dark:border-white/10 dark:bg-white/5">
      <button
        aria-label="Upvote"
...
function AnswerItem({
  ...
  return (
    <div className={depth > 0 ? "border-l border-slate-200 pl-4 md:pl-5" : ""}>
      <article className="app-card rounded-[1.65rem] p-5">
        ...
        <button
          className="app-button-secondary mt-5 inline-flex min-h-[44px] items-center gap-2 rounded-2xl px-4 text-xs font-semibold transition-all duration-200 hover:text-gray-900"
          type="button"
...
export function QuestionThread({ questionSlug }: { questionSlug: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="relative overflow-hidden rounded-[1.9rem] border border-[#dbe7f5] bg-[#f0f5fd] p-5 shadow-[0_14px_42px_rgba(45,76,112,0.05)] dark:border-blue-500/10 dark:bg-blue-500/10 md:p-7">
        <span className="absolute inset-y-7 left-0 w-1.5 rounded-r-full bg-[#6e9fe0]" />
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-[#dff4ec] px-3 py-1.5 text-xs font-semibold text-[#0e7a65] dark:bg-teal-500/10 dark:text-teal-300">
                {question.courseCode}
              </span>
          ...
          <div className="grid min-w-[220px] grid-cols-2 divide-x divide-[#dbe7f5] overflow-hidden rounded-2xl border border-white/80 bg-white/70 dark:divide-white/10 dark:border-white/10 dark:bg-white/5">
            <div className="p-4">
